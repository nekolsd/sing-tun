...
func TestForwardNATConcurrentExhaustionAcrossStages(t *testing.T) {
	...
	wg.Wait()
	requireNoLeakedReservations(t, d.natFor(port))
}

// requireNoLeakedReservations fails if a selector is still reserved (a nil
// placeholder in the reverse table) without a published flow.
func requireNoLeakedReservations(t *testing.T, nat *portNAT) {
	t.Helper()
	for i := range nat.shards {
		shard := &nat.shards[i]
		shard.access.RLock()
		for key, flow := range shard.flows {
			require.NotNil(t, flow, "selector reservation leaked for %v", key)
		}
